...
// MARK: - UIBarButtonItem + AnchorView
...
public final class DropDown: UIView {
    ...
    public enum Direction {
        /// The drop down will show below the anchor view when possible, otherwise above if there is more place than
        /// below.
        case any
...
extension DropDown {
    ...
    private func computeLayout()
        -> (x: CGFloat, y: CGFloat, width: CGFloat, offscreenHeight: CGFloat, visibleHeight: CGFloat,
            canBeDisplayed: Bool, Direction: Direction)
    {
        ...
        barButtonItemCondition: if let anchorView = anchorView as? UIBarButtonItem {
            ...
            let width = viewWidth ?? fittingWidth()
    ...
    private func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        let width = viewWidth ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - bottomOffset.x
    ...
    private func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        let width = viewWidth ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - topOffset.x
    ...
    private func fittingWidth() -> CGFloat {
        if templateCell == nil {
            templateCell = DropDownCell(style: .default, reuseIdentifier: DPDConstant.ReusableIdentifier.DropDownCell)
        }
...
public extension DropDown {
    ...
    func show(
        ...
        if self == DropDown.VisibleDropDown,
           ...
           false
        {
            // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue
            // when continuous taping on button - Kartik Patel - 2016-12-29
            return (true, 0)
        }
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(
            withIdentifier: DPDConstant.ReusableIdentifier.DropDownCell,
            for: indexPath
        ) as? DropDownCell ?? DropDownCell(
            style: .default,
            reuseIdentifier: DPDConstant.ReusableIdentifier.DropDownCell
        )
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        cell.customSeparator.backgroundColor = separatorColor
        cell.customSeparator.isHidden = index == dataSource.count - 1
